...
#Creating a logger "tuc logger" to write to file
logconfpath = "myeditorlog.conf";
log_file_path = path.join(path.dirname(path.abspath(__file__)), logconfpath);
logging.config.fileConfig(log_file_path, disable_existing_loggers=False);
logger = logging.getLogger('tuc_logger');
...
def load_tweets():
    ...
    logger.info("Loaded the file into memory.") 
    tweet_count = 0;
    for line in f:
        tweet_count += 1;
    f.seek(0);
...
def update_tweet(id: int) -> int:
    #Logging message
    logger.info("A tweet was updated.")
...
def delete_tweet() -> int:
    #Logging message
    logger.info("A tweet was deleted.")
...
def read_last_tweet() -> int:
    #Logging message
    logger.info("Last tweet was read.")
...
def read_one_up() -> int:
    #Logging message
    logger.info("One tweet up was read.")
...
def read_one_down() -> int:
    #Logging message
    logger.info("One tweet down was read.")
...
def print_current_tweet_id() -> int:
    #Logging message
    logger.info("Current tweet was printed.")
...
def quit_without_save() -> int:
    #Logging message
    logger.info("Quit program without saving.")
...
def save_and_exit() -> int:
    #Logging message
    logger.info("Exited program after saving.")
