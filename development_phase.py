# WE NEED TO INCLUDE THE NON STANDARD PACKAGES IN THE MAKEFILE
...
current_tweet = 1;
# CURRENT TWEET REFERS TO THE CURRENT LINE (starting at 1)
...
def user_choice_to_int_list(user_choice: str) -> list:
    ...
    #if there's a space in the user's input then the user probably selected either "r" or "u"
...
# with said function's return value being checked for errors (to avoid clutter error checker has been disabled, doing nothing)
...
def load_tweets():
    ...
    # proggress bar functionality. I'll leave it upon the refactoring team to figure this one out. The project requirments don't call for a progress bar, but I've
    # included one in the spirit of good user experience
    tweet_count = 0;
    for line in f:
        tweet_count += 1;
    f.seek(0);
...
def read_one_down() -> int:
    # if current_tweet is 1 calling read_one_down() would read tweets[-1]
    if current_tweet == 0:
